...
def get_files():
    ...
    prev_total_df = pl.read_excel(
        prev_file, sheet_name="Total", columns=[0], **read_args
    )
    prev_summary_df = pl.read_excel(
        prev_file, sheet_name="Summary", columns=[1, 4], **read_args
    )
    prev_apdetail_df = pl.read_excel(
        prev_file, sheet_name="AP Detail", columns=[1, 3, 5, 6], **read_args
    )

    new_total_df = pl.read_excel(new_file, sheet_name="Total", columns=[0], **read_args)
    new_summary_df = pl.read_excel(
        new_file, sheet_name="Summary", columns=[1, 4], **read_args
    )
    new_apdetail_df = pl.read_excel(
        new_file, sheet_name="AP Detail", columns=[1, 3, 5, 6], **read_args
    )
...
def get_amounts():
    ...
    total_amount_diff = (
        min(prev_total_amount, new_total_amount)
        / max(prev_total_amount, new_total_amount)
        * 100
    )
...
def final_validation():
    ...
    amount_valid = (
        min(new_total_amount, prev_total_amount)
        / max(new_total_amount, prev_total_amount)
        * 100
    ) > 35
    ...
    is_final_validation = (
        client_matches
        and carrier_matches
        and are_dates_correct
        and amount_valid
        and late_payment_amount_valid
        and there_no_dupes
        and glcodes_valid
    )
...
def change_file_name(finalValidation: bool):
    if finalValidation:
        ...
        if is_sarnova:
            os.rename(
                new_file,
                f"Sarnova {new_carrier_name.upper()} Parcel Cost Report {new_client_name} WE{date_formatted}.xlsx",
            )
            return
        os.rename(
            new_file,
            f"{new_client_name} {new_carrier_name.upper()} Parcel Cost Report WE{date_formatted}.xlsx",
        )
    else:
        if is_sarnova:
            os.rename(
                new_file,
                f"REVIEW - Sarnova {new_carrier_name.upper()} Parcel Cost Report {new_client_name} WE{date_formatted}.xlsx",
            )
            return
        os.rename(
            new_file,
            f"REVIEW - {new_client_name} {new_carrier_name.upper()} Parcel Cost Report WE{date_formatted}.xlsx",
        )
